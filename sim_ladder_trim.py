...
SPECS=[
    ("ladder full (yours)",      lambda: S.EricLadder()),
    ("ladder trim (no yo, inside, DC30)", lambda: S.EricLadder(yo=False, place_410=False, dc=30)),
...
if __name__=="__main__":
    ...
    print(f"{'strategy':<38}{'mean$':>7}{'med$':>6}{'edge%':>7}{'win%':>6}{'bust%':>6}{'p05':>7}{'p95':>6}{'sd':>5}{'maxDD':>7}")
    print("-"*97)
    for i,(label,_) in enumerate(SPECS):
        nets,acts,busts,troughs=agg[i]; nets.sort(); n=len(nets)
        m=statistics.mean(nets)
        print(f"{label:<38}{m:>7.1f}{nets[n//2]:>6.0f}{-m/statistics.mean(acts)*100:>7.3f}"
